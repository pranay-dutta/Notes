...
  void insert(string& word) { //TC: O(L)
...
  int countWordsEqualTo(string& word) { //O(L)
    TrieNode* crawler = root;
    for (char ch : word) {
      if (!crawler->containsKey(ch))  return 0;
      crawler = crawler->get(ch);
    }
    return crawler->getEnd();
  }

  int countWordsStartingWith(string& prefix) { //O(L)
    TrieNode* crawler = root;
    for (char ch : prefix) {
      if (!crawler->containsKey(ch)) return 0;
      crawler = crawler->get(ch);
